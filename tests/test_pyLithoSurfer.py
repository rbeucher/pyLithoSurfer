...
def test_shrimp_info():
    shrimp_instance = SHRIMPDataPoint(id=638451)
    response = shrimp_instance.info()
    assert isinstance(response, dict)
    assert response['id'] == 638451, "The ID should be in response"
...
def test_shrimp_data_create():
    ...
    response = shrimp_instance.push_new_entry()
    assert isinstance(response, dict)
    assert "id" in response.keys()
    assert set(shrimp_keys()).issubset(list(response.keys())), "All keys should be in the response"
...
def test_shrimp_data_update():
    ...
    response = shrimp_instance.update_entry()
...
def test_shrimp_data_delete_entry():
    shrimp_instance = SHRIMPDataPoint(**shrimp_example())
    response = shrimp_instance.push_new_entry()
    response = shrimp_instance.delete_entry()
    assert isinstance(response, dict)